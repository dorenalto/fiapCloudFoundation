...
describe('Test suite for Musician store REST API', () => {
  ...
  test('It should modify an existing musician via the PUT method - failed case', async () => {
    const musician = {
      firstName: "BB",
      lastName: "King",
      genre: "ROCK"
    };
    const res = await request(server).put('/musician/BB').send(musician);
    const responseBody = JSON.parse(res.text);
    expect(res.status).toEqual(400);
    expect(responseBody).not.toBeNull();
    expect(responseBody).toHaveProperty('id');
    expect(responseBody.id).toEqual('BB');
  });
